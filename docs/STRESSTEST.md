...
Ezen felül a Github Actions workflow fájlba felvettem a következőket, hogy a CI/CD pipeline részeként a locust alkalmazás is frissüljön:
```yaml
- name: Update deployments with new images
        run: |
          [...]
          LOCUST_DIGEST=$(oc get istag fotoalbum-locust:latest -n fotoalbum -o jsonpath='{.image.dockerImageReference}')

          
          [...]
          oc set image deployment/fotoalbum-locust fotoalbum-locust=$LOCUST_DIGEST -n fotoalbum
```
...
<!-- TODO: aut. felskálázódás bizonyítéka -->
...
Az OpenShift felületén látható, hogy a tesztelés megkezdése után megugrottak a memória- és processzor kihasználtságok, aminek következtében új Pod-ok indultak. A türkiz görbe az első Pod terhelését mutatja. Látszik, hogy ez kapta a legnagyobb terhelést a többi közül. Ennek oka, hogy a hirtelen történt terhelésnövekedés ugyan új Pod-okat indított, de ezek elkészüléséig csakis az első Pod kapta a terhelést. 
